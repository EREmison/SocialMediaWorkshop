...
cat('\n\n\nThere are', length(files), 'files in directory ', getwd(), '.', '\n\n\n\n')
...
leanParseTweets <- function(tweets, simplify=FALSE, verbose=TRUE){
  ...
  ## from json to list
  results.list <- readTweets(tweets, verbose=TRUE)
...
# Remove retweets
dfrm <- dfrm %>% 
  filter(retweeted == FALSE & lang == 'en' & user_lang == 'en') %>% 
  filter(!grepl("^rt|^RT", text))
if("retweeted" %in% names(dfrm)){
  dfrm <- dfrm %>% select(-retweeted)
}


osize <- format(object.size(dfrm), units='auto')
cat('\ndata frame size (tweets):', osize)


# Write data
write_csv(dfrm, outname)
cat('\n\nwrote file', outname, 'to disk\nit has', nrow(dfrm), 'rows and', ncol(dfrm), 'columns\n')
filesize <- file.info(outname)$size %>% utils:::format.object_size('auto')
cat('it has', filesize, '\n\n')
rm(dfrm)
